...
@Component({
  ...
  template: `
    ...
    <div class="absolute inset-0 z-40" (click)="cancelled.emit()">
      <div
        class="absolute grid grid-cols-2 gap-1 rounded-xl border border-[#d4b0bc] bg-[#fdf6e8] p-1.5 shadow-2xl"
      ...
      >
        @for (piece of pieces; track piece) {
          <button
            ...
            class="flex items-center justify-center rounded-lg bg-white p-1 transition-colors hover:bg-[#fce8ec]"
